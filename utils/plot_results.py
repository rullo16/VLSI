...
def plot_solution(w_plate, h_plate, n, circuits, solution, legend=True, colors=None, save_path=None):
    """
    ...
    if save_path is not None:
        plt.savefig(f"{save_path}/{w_plate}x{h_plate}-sol.png", dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--filename", help="Filename of the output", required=True, type=str)
    parser.add_argument("-l", "--legend", help="Enable legend, otherwise number of circuits showed on the blocks", required=False, action='store_true')
    args = parser.parse_args()

    if not path.isfile(args.filename):
        print("\nSpecified file doesn't exist, insert an existing solution file.\n")
    else:
